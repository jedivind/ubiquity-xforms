...
function _getEvaluationContext(pThis, nOrdinal) {
    ...
    /*
     * If we have a bind attribute then use it to find the model and evaluation
     * context.
     */
    ...
    if (sBindID) {
        ...
        if (pThis.element.getAttribute("model")) {
            ...
            var oModel = pThis.element.ownerDocument
                    .getElementById(pThis.element.getAttribute("model"));
            // TODO: The form author cannot be relied upon to provide an IDREF
            // in @model that actually corresponds
            // to a model. In order to prevent errors throwing out and causing
            // problems, a test is required to ensure that
            // oModel, retrieved above does not only exist (as the following
            // test proves) but is also actually a model
            // (a possibility which is ignored by the following test)
            // (The obvious test of using a tagname for pThis should not be done
            // as the behaviours should not need to know
            // what they are bound to, i.e. a node may behave as a model even
            // without being a <model>)
            
            if (oModel) {
                // Having fetched a model node which corresponds to the given
                // @model IDREF
                // Find the model to which the parent element is bound.
                var oCTXModel = getModelFor(pThis.element.parentNode);
                // In the case that the parent's model and the model fetched
                // from the @model IDREF
                // are identical, the evaluation context for pThis node is the
                // context gleaned
                // from its position within the document, to wit, the same
                // context as though it
                // had no model attribute at all.
                if (oCTXModel === oModel) {
                    oRet = _getParentEvaluationContext(pThis);
                }
                // Where the above clause is false, i.e. a disparity exists
                // between the model to which
                // the parent node is bound, and the model to which pThis node
                // is bound, then pThis node is
                // not evaluated in the context of the parent node, but is
                // evaluated in the default context for
                // the model whose id matches the IDREF given in pThis element's
                // model attribute.
                else {
...
function _getParentEvaluationContext(pThis, nOrdinal) {
    ...
    while (oParent) {
        if (oParent.getBoundNode === undefined) {
            ...
            if (!oRet || (!oRet.model && !oRet.node)) {
                ...
                // invoking it at this point leads inexorably to a stack
                // overflow
    ...
    /*
     * If we don't get a context then we must be the highest element, so we use
     * the evaluation context of the first model.
     */

    if (!oParent) {
        // retrieve the first model in document order.
        if (!document.defaultModel) {
            var models = NamespaceManager.getElementsByTagNameNS(
                    pThis.element.ownerDocument,
...
function _getBoundNode(pThis, nOrdinal) {
    ...
    if (pThis.m_proxy && !pThis.m_proxy.m_xpath) {
        ...
        if (pThis.element.getAttribute("bind")) {
            if (!pThis.m_arrNodes) {
                var oBind = pThis.element.ownerDocument
                        .getElementById(pThis.element.getAttribute("bind"));
            ...
            // Don't bother going through all pThis palaver if there is no ref
            // or nodeset.
            
            if (pThis.element.getAttribute("ref") || 
                pThis.element.getAttribute("nodeset")) {
                /*
                ...
                if (pThis.element.getAttribute("ref") && nOrdinal === 1) {
                    oRet.node = getFirstNode(
                                pThis.m_model.EvaluateXPath(
                                    pThis.element.getAttribute("ref"), 
                                    oContext.node));
                    oRet.model = pThis.m_model;
                } else if (pThis.element.getAttribute("nodeset")) {
                    if (!pThis.m_arrNodes)
                        pThis.m_arrNodes = pThis.m_model.EvaluateXPath(
                                pThis.element.getAttribute("nodeset"),
                                oContext.node).value;
