...
//				this.initHeader('Content-Type',' application/xml; charset=UTF-8 ');
//				this.initHeader('Cookie', 'SITESERVER=ID=15c6d997b0c069073be00f82c0ac8eb1');
				//postData = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"  standalone=\"no\" ?><x>" + postData + "</x>";
				var x = new ActiveXObject("MSXML2.DOMDocument");
				x.loadXML("<postData/>");
				x.documentElement.text = "bumhole";
				postData = x;
				
				
/*				this.initHeader('Content-Type','text/xml;charset=ISO-8859-1');
				this.initHeader('Pragma','no-cache');
				this.initHeader('Proxy-Connection','Keep-Alive');
				//this.initHeader('Content-Length', '395');
				var arrNewLines = new Array();
				for(var i = 0; i < 350; ++i)
				{
					arrNewLines.push("\n")
				}
				postData += arrNewLines.join("");
				postData += "<!--The End-->";
				postData += "\n\n";
*/			}
