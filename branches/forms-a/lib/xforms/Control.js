...
function Control(elmnt) {
    ...
    this.m_TVCListenerAdded = false;
    this.m_value = null;
    this.m_isWebFormA = false;
}
...
Control.prototype.RetrieveValuePseudoElement = function() {
    ...
    // When value is not define, determine it
    // Get the namespace of the control
    ...
    if (sNs === "http://www.w3.org/2002/xforms") {        
        ...
        if (faName) {
            ...
            if (UX.isIE) {
                ...
                oElement.addEventListener("change",function(e){valueChangedFF(oElement,e);},false);
            }
...
Control.prototype.AddTVCListener = function() {
    // Any implementation of pe-value must provide an 
    // event that tells us the data has changed.
    if (!this.m_value) {
    ...
    if (this.m_value.addEventListener) {
        ...
        if (UX.isIE) {
            valueChangedCB.handleEvent = function(evt) {
                evt.type = "target-value-changed";
                FormsProcessor.dispatchEvent(this.control, evt);                    
            };
        } else {
            valueChangedCB.handleEvent = function(evt) {
                var oEvt = this.control.element.ownerDocument.createEvent(
                        "MutationEvents");
        ...
        this.m_value.addEventListener("control-value-changed", valueChangedCB, false);
        this.m_TVCListenerAdded = true;
    }
};
