...
function mapclick2domactivate(elmnt, e) {
    ...
    // There is no need to run this event in line, and doing so may cause a
    // stack overflow,
...
function findEvnetListener(oArray, oListener) {
...
if (document.all) {
    ...
    function _addEventListener(sType, oListener, bPhase) {
        if (typeof(sType) !== "string" || typeof(bPhase) !== "boolean" || !oListener ) {
            ...
            var arr = this.arrListener[sType][this.PHASE_BUBBLE];
            return;
        }

        var  iPhase = (bPhase) ? this.PHASE_CAPTURE : this.PHASE_BUBBLE;
        /*
         * If this is the first listener of this type then create an empty list
         */        
        if (!this.arrListener[sType]) {
         ...
         */
        if (!this.arrListener[sType][iPhase]) {
            this.arrListener[sType][iPhase] = new Array();            
            // First listener just add it, no need to check            
            var i = this.arrListener[sType][iPhase].push(oListener);
            return;
        }

        // listener not found
        if (findEvnetListener(this.arrListener[sType][iPhase], oListener) === -1) {
    ...
    function _removeEventListener(sType, oListener, bPhase) {        
         ...
         */
        if (!this.arrListener[sType][iPhase]) {
            // No listener exist, just return 
            return;
        }
         
        var oList = this.arrListener[sType][iPhase];
        // Find event listener 
        var idx = findEvnetListener(oList, oListener);
    ...
    function __notifyListeners(oEvt) {
        ...
        if (arr) {
            /*
             * Next, narrow it down to just those listeners for the correct
             * phase
             */
            ...
            if (arr && arr.length) {
                ...
                /*
                 * If we have a list of listeners then invoke their handlers
                 */
                ...
                for ( var i = 0; i < arr.length; i++) {
                    ...
                    // either in the previous iteration, or before the first
                    // iteration.
                    FlushEventQueue();
                    var oListener = arr[i];
                    var bInvoke = true;
                    ...
                    /*
                     * If @ev:preventDefault is set then the default handler is
                     * to be cancelled.
                     */
                    ...
                    /*
                     * If @ev:propagate is set then we need to stop propagation.
                     */
                    ...
                    /*
                     * Stop propagation doesn't stop a group, but "stop
                     * immediate" does
                     */
                ...
                // flush any events that were added to the queue by the last
                // iteration.
                FlushEventQueue();
                ...
                /*
                 * Stop propagation let's the current group complete before
                 * stopping all other listeners
                 */
    ...
    function capture(oEvt, arrTargetList) {
        /*
         * CAPTURE PHASE In the capture phase we target ancestors in order from
         * the root to our target.
         * 
         * After notifying each listener we check oEvt._stopPropogation to see
         * if the process should continue, although for each target we carry out
         * all listeners, uninterrupted.
         * 
         * Note that capturing does *not* happen on the target of the event.
         */
    ...
    function bubble(oEvt, arrTargetList) {
        /*
         * BUBBLE PHASE In the bubble phase we target ancestors in reverse
         * order, i.e., from the target up to the root. Begin with the 'current'
         * target.
         */
        if (!oEvt._stopPropagation) {
    ...
    function __dispatchEvent(oEvt) {
        ...
        try {
            ...
            /*
             * The 'target' is always the same, although the 'currentTarget'
             * will change as bubbling and capture take place.
             */
            ...
            /*
             * Increase the action depth, since we don't want to update the
             * models until we exit the top-level action handler.
             */
