...
XF4HProcessor = {    
    ...
    */    
    getAttribute : function(oElement, sAttributeName) { 
        ...
        if (!sAttrValue) {
            ...
            sAttrValue = oElement.getAttribute(sAttributeName);
        }    
        return sAttrValue;
    },

    createPseudoValue : function (oElement) {
        var oPseudo = null;
        var sDataType = null;
        
        if (oElement.tagName === "INPUT") {
            oPseudo = new XF4HInputValue(oElement);
        } else if (oElement.tagName === "OUTPUT") {
            oPseudo = new XF4HOutputValue(oElement);
        }  else if (oElement.tagName === "SPAN") {             
            oPseudo = new XF4HOutputValue(oElement);
        }
        return oPseudo;
    },
    ...
    */    
    processElement : function(oModel, oContextNode, oElement, sName) {
        ...
        // console.log("process name=" + sName);
    ...
    */    
    _createBind : function(oModel, oElement, oRefNode) {
        ...
        if (!UX.hasDecorationSupport || oElement.isXF4H()) {
