...
FunctionCallExpr.prototype.xpathfunctions["days-to-date"] = function(ctx) {
	...
	var days = Math.round(number);
	// Set the date by working out the number of milliseconds since the *end* of 1970-01-01.
	var dOrigin = new Date();
	dOrigin.setTime(days * 24 * 60 * 60 * 1000);
...
FunctionCallExpr.prototype.xpathfunctions["expr"] = {
  xsdDate: /^([0-9]{4})\-([0-9]{2})\-([0-9]{2})\T([0-9]{2})\:([0-9]{2})\:([0-9]{2})(((([+-])([0-9]{2})\:([0-9]{2}))|(\Z))?)$/
};
...
FunctionCallExpr.prototype.xpathfunctions["adjust-dateTime-to-timezone"] = function(ctx) {
  ...
  //
	var res = FunctionCallExpr.prototype.xpathfunctions.expr.xsdDate.exec(
