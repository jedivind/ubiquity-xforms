(function() {
	...
	var suite = new YAHOO.tool.TestSuite({
		...
		setUp : function() {
			...
			this.originInstance = new Instance( this.testDIV );
	...
	suite.add(
		new YAHOO.tool.TestCase({
            ...
            testInsertElementPrototypeIntoEmptyList: function() {
                ...
                // newly inserted node belongs to the right document.
                // 
                Assert.areEqual(1, 
                    suite.testInstance.evalXPath(
                        'count(/po/order)',
                        suite.testInstance.evalXPath('order/item[1]').nodeSetValue()[0] 
                    ).numberValue(), 
                    "XPath failed because owner document of inserted node is not correctly set");
