...
function _getBoundNode(pThis, nOrdinal) {
    ...
    if (oProxy && !oProxy.m_xpath) {
        ...
        if (!oModel) {
            oModel = _getEvaluationContext(pThis).m_model;
            pThis.m_model = oModel;
        }
        return { model : oModel, node: oProxy };
    }
    ...
    var oElement = pThis.element;
    var sBindId  = oElement.getAttribute("bind");
    var oBindNodes = pThis.m_arrNodes;
    ...
    if (sBindId) {
        if (!oBindNodes) { 
            ...
            oModel     = oBind["ownerModel"];
            oBindNodes = oBind["boundNodeSet"];
            pThis.m_model = oModel;             
            pThis.m_arrNodes = oBindNodes;            
        } 
        
        return { model : oModel, node: oBindNodes[nOrdinal - 1] };
    }
    ...
    if (sRef && nOrdinal == 1) {        
        var oRefNode = 
            getFirstNode(oModel.EvaluateXPath(sRef, oRet.node));
        ...
        if (!oRefNode) {
            ...
            if (oInstDoc) {
                ...
                oRefNode = getFirstNode(oModel.EvaluateXPath(sRef, oRet.node));
        ...
        if (!oBindNodes) {
            oBindNodes = oModel.EvaluateXPath(sNodeset, oRet.node).value;
            // Form controls are considered to be non-relevant if any of the 
            // following apply:
            // the Single Node Binding is expressed and resolves to empty nodeset
            // so oRefNode is null if EvaluateXPath is unresolved.
        ...
        oRet.node = oBindNodes[nOrdinal - 1];
    }    
    return oRet;
}
