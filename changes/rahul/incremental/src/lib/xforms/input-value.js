...
XFormsInputValue.prototype.onDocumentReady = function()
{
	if (this.element.ownerDocument.media != "print")
	{
		...
		var pThis = this;
        if (this.element.parentNode.getAttribute("incremental") === "true") {
        	if (document.all) {
        		oInput.attachEvent("onkeyup", function(e){valueChangedIE(pThis,e);});
	        } else {
		        oInput.addEventListener("keyup",function(e){valueChangedFF(pThis,e);},false);
        	}
        } else {
        	if(document.all) {
