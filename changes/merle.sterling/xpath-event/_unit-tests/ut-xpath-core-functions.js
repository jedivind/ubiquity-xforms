...
suiteXPathCoreFunctions.add(
  new YAHOO.tool.TestCase({
    ...
    testEvent : function () {
      ...
      // Create an Instance element to use as an event target and attach an event listener.
      this.testDIV = document.createElement( "div" );
      this.testInstance = new Instance( this.testDIV );
      if (UX.isIE) {
          this.testInstance.element.attachEvent("test-event", function(evt) {
              Assert.areEqual(this.nodesetProperty, evalXPath("event('nodeset-property')").nodeSetValue());
              Assert.areEqual(this.stringProperty, evalXPath("event('string-property')").stringValue());
              Assert.areEqual(this.numberProperty, evalXPath("event('number-property')").stringValue());
              Assert.isTrue(this.booleanProperty, evalXPath("event('boolean-property')").booleanValue());
          }, false);
      } else {
          this.testInstance.element.addEventListener("test-event", function(evt) {
              Assert.areEqual(this.nodesetProperty, evalXPath("event('nodeset-property')").nodeSetValue());
              Assert.areEqual(this.stringProperty, evalXPath("event('string-property')").stringValue());
              Assert.areEqual(this.numberProperty, evalXPath("event('number-property')").stringValue());
              Assert.isTrue(this.booleanProperty, evalXPath("event('boolean-property')").booleanValue());
      ...
      // boolean and add them to an event.
      this.nodesetProperty = evalXPath('/test/numbers').nodeSetValue();
      this.stringProperty = evalXPath('/test/numbers2[0]').stringValue();
      this.numberProperty = evalXPath('/test/numbers[0]').numberValue();
      this.booleanProperty = evalXPath('/test/numbers[0]').booleanValue();

      // Fire an event with attached context info.
      var evt = document.createEvent("Events");
      evt.initEvent("test-event", true, false);
      evt.context = {
          "nodeset-property" : this.nodesetProperty,
          "string-property" : this.stringProperty,
          "number-property" : this.numberProperty,
          "boolean-property" : this.booleanProperty
      };
      FormsProcessor.dispatchEvent(this.testInstance, evt);
    }
