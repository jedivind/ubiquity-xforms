...
Selenium.prototype.assertXFormsCSSProperty = function(locator, property)
{
    var currentDocument = selenium.browserbot.getCurrentWindow().document;
    var element = this.page().findElement(locator);
    var i, arrSegments, bFound = false;
    ...
    if(!element)
    {
        ...
        arrSegments = element.className.split(" ");
        for (i = 0; i < arrSegments.length; i++) {
            if (arrSegments[i] === property) {
                bFound = true;
                break;
            }
        }
    ...
    if (!bFound) {
        Assert.fail("element does not have style property '" + property + "'");
    }
}
