...
Submission.prototype.onDocumentReady = function()
{
	  ...
	  this,
		"xforms-submit",
		{
			handleEvent: function(evt) {   
                var oEvt;
                if (evt.target.submissionInProgress) {
                    oEvt = evt.target.ownerDocument.createEvent("Events"); 
                    oEvt.initEvent("xforms-submit-error", true, false);
                    oEvt.context = {
                        "error-type" : "submission-in-progress"
                    };
                    FormsProcessor.dispatchEvent(evt.target, oEvt);
                } else {
                    evt.target.submissionInProgress = true;
	...
	FormsProcessor.addDefaultEventListener(
	  this,
		...
		{
			handleEvent: function(evt) {   
                evt.target.submissionInProgress = false;
			}
		}
	);
