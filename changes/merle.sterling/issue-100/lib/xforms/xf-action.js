...
function XFAction(elmnt)
{
	this.element = elmnt;
		...
		XFAction.prototype.handleEvent = function(evt) {
		    /*
		    * An action handler simply supports a handleEvent method,
		    * so loop through executing them all.
		    */

            var ifExpr = this.element.getAttribute("if");
            if (ifExpr) {
		        var oContext = this.element.getEvaluationContext();
		        var oRes = oContext.model.EvaluateXPath(ifExpr, oContext.node).booleanValue();
		        if (!oRes) {
		            return;
		        }
		    }
