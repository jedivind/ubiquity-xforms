...
submission.prototype.processResult = function(oResult, isFailure, 
    ...
    var sData, sReplace, sInstance, oEvt, oNewDom, contentType = "", sTarget, oTargetContext, oTarget;
        ...
        if (isFailure) {
            ...
            if (sData) {
                ...
                case "instance":
                    ...
                    if (sTarget) {
                        oTargetContext = sInstance ? oContext.model.getInstanceDocument(sInstance).documentElement : oContext.node; 
                        oTarget = oContext.model.EvaluateXPath(sTarget, oTargetContext).nodeSetValue()[0];
                        if (!oTarget || oTarget.nodeType !== DOM_ELEMENT_NODE || oTarget.parentNode.getAttribute("readonly")) {
                            oEvt.context["error-type"] =  "target-error";
                            oEvt.initEvent("xforms-submit-error", true, false);
                            FormsProcessor.dispatchEvent(oObserver, oEvt);
                        } else if (oTargetContext == oTarget) {
