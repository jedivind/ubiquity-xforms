...
Control.prototype.setView = function () {
  this.testMIPChanges();
  setState(this, null, "enabled", "enabled", "disabled");
  setState(this, null, "readonly", "read-only", "read-write");
  setState(this, null, "required", "required", "optional");
  setState(this, null, "valid", "valid", "invalid");
};

Control.prototype.testMIPChanges = function () {
  if (this.isDirtyMIP("enabled")) {
    this.dirtyState.setDirty("enabled");
  }
  if (this.isDirtyMIP("readonly")) {
    this.dirtyState.setDirty("readonly");
  }
  if (this.isDirtyMIP("required")) {
    this.dirtyState.setDirty("required");
  }
  if (this.isDirtyMIP("valid")) {
    this.dirtyState.setDirty("valid");
  }

};

Control.prototype.isDirtyMIP = function (sMIPName) {
...
Control.prototype.dispatchMIPEvents = function () {
  if (this.m_proxy) {  
    UX.dispatchEvent(this, this.m_proxy.valid.getValue() ? "xforms-valid" : "xforms-invalid", true, false);
    UX.dispatchEvent(this, this.m_proxy.required.getValue() ? "xforms-required" : "xforms-optional", true, false);
    UX.dispatchEvent(this, this.m_proxy.readonly.getValue() ? "xforms-readonly" : "xforms-readwrite", true, false);
  }
  UX.dispatchEvent(this, this.isEnabled() ? "xforms-enabled" : "xforms-disabled", true, false);
};

Control.prototype.mustBeBound = function () {
	return true;
};

Control.prototype.isEnabled = function () {
	if (this.m_proxy) {
		return this.m_proxy.enabled.getValue();
	}

	return this.mustBeBound() ? false : true;
};

Control.prototype.getMIPState = function (mip) {
	var retval = { isSet: false };

	if (mip === "enabled") {
		retval.value = this.isEnabled();
		retval.isSet = true;
	} else if (this.m_proxy) {
		retval.value = this.m_proxy.getMIPState(mip);
		retval.isSet = true;
	}
