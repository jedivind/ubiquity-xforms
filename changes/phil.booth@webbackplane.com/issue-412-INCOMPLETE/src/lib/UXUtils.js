...
UX.areSame = function (firstItem, secondItem) {
	return firstItem === secondItem;
};

UX.listenForHTMLKeyDownEvent = function (self, targetElement) {
	var onKeyDown = function (evt) {
		self.onKeyDown(evt);
	};

	UX.addHTMLEventListener(targetElement, 'keydown', UX.createHTMLEventHandler(onKeyDown));
};
...
UX.addHTMLEventListener = function (targetElement, eventName, handlerFunction) {
	if (targetElement.attachEvent) {
		return targetElement.attachEvent('on' + eventName, handlerFunction);
	}

	if (typeof targetElement.addEventListener === 'function') {
		targetElement.addEventListener(eventName, handlerFunction, false);
		return true;
	}
...
UX.createHTMLEventHandler = function (handlerFunction) {
	return function (eventArgument) {
		return handlerFunction(UX.getHTMLEvent(eventArgument));
	};
};
...
UX.isTabKeyIdentifier = function (keyIdentifier) {
	return UX.getTabKeyIdentifier() === keyIdentifier;
};

UX.getTabKeyIdentifier = function () {
	return 'U+0009';
};
