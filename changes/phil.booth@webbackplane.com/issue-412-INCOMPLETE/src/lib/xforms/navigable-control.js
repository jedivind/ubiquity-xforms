...
NavigableControl.prototype.listenForKeyDownEvent = function () {
//	UX.listenForHTMLKeyDownEvent(this, this.element);
	var self = this;
	if (typeof this.element.addEventListener === 'function') {
		this.element.addEventListener('keydown', { handleEvent: function (evt) { self.onKeyDown (evt); } }, false);
	}
};
...
//};
NavigableControl.prototype.onKeyDown = function (evt) {
	if (UX.isTabKeyIdentifier(evt.keyIdentifier)) {
		if (evt.shiftKey) {
			this.navigateToPreviousControl();
		} else {
			this.navigateToNextControl();
