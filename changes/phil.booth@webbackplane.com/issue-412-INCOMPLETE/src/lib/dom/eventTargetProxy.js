...
if (UX.isIE) {
    ...
    function EventTargetProxy(elmnt) {
        ...
        if (this.isNavigableControl) {
			this.element.onkeydown = function (evt) {
				var keyboardEvent;
				// TODO: Consider doing away with this test, if we can get an automatic conversion from keyCode number to keyIdentifier string (e.g. 9 => 'U+0009').
				if (UX.isTabKey(evt.keyCode)) {
					keyboardEvent = document.createEvent('KeyboardEvents');
					keyboardEvent.initKeyboardEvent('keydown', true, true, null, 'U+0009', KeyboardEvent.DOM_KEY_LOCATION_STANDARD, UX.isShiftKeyPressed(evt) ? 'Shift' : '');
					this.element.dispatchEvent(keyboardEvent);
					// TODO: Test this code then handle the KeyboardEvent in NavigableControl etc.
					// TODO: Consider event cancellation options, I don't think we return the correct value from dispatchEvent.
				}
			}
        }
    }
