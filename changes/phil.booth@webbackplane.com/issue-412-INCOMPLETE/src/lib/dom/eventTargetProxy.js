...
if (UX.isIE) {
    ...
    function EventTargetProxy(elmnt) {
		...
		this.element.onkeydown = function () {
			...
			if (this.isNavigableControl && UX.isTabKeyCode(event.keyCode)) {
				keyboardEvent = document.createEvent('KeyboardEvents');
				keyboardEvent.initKeyboardEvent('keydown', true, true, null, UX.getTabKeyIdentifier(), KeyboardEvent.DOM_KEY_LOCATION_STANDARD, UX.isShiftKeyPressed(event) ? 'Shift' : '');
				if (!this.element.dispatchEvent(keyboardEvent)) {
   ...
   var flushEventQueue = function() {
    ...
    EventTarget = function(elmnt) {
        ...
        this.element.addEventListener("keydown", function(evt) {
            if (this.isNavigableControl && UX.isTabKeyCode(evt.keyCode)) {
				if (!this.propagatingTabKeyDown) {
					this.propagatingTabKeyDown = true;
					// TODO: Debug and see why this isn't working (suspect createEvent).
					keyboardEvent = document.createEvent('KeyboardEvents');
					keyboardEvent.initKeyboardEvent('keydown', true, true, null, UX.getTabKeyIdentifier(), KeyboardEvent.DOM_KEY_LOCATION_STANDARD, UX.isShiftKeyPressed(evt) ? 'Shift' : '');
					if (!this.element.dispatchEvent(keyboardEvent)) {
    					evt.preventDefault();
	    				evt.stopPropagation();
					}
					this.propagatingTabKeyDown = false;
				}
		    }
        }, false);
   };
