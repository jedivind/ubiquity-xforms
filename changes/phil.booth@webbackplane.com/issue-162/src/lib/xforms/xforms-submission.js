            ...
            if (sData) {
                ...
                case "all":
                    ...
                    if (UX.isXHTML) {
                      ...
                      newHtml.innerHTML = sData;
                      document.documentElement.innerHTML = newHtml.getElementsByTagName("html")[0].innerHTML;
                    } else {
...
submission.prototype.submit = function(oSubmission) {
    ...
    var submitDataList = [ ];
    var oForm;
    var omitXMLDeclaration = oSubmission.getAttribute("omit-xml-declaration") ? oSubmission.getAttribute("omit-xml-declaration") === "true" || oSubmission.getAttribute("omit-xml-declaration") === "1" : false;
    var standalone = oSubmission.getAttribute("standalone") ? oSubmission.getAttribute("standalone") : false;
	...
	case "get":
		...
		oBody = this.serializeSubmitDataList(submitDataList, sSerialization, sEncoding, omitXMLDeclaration, standalone);
		break;
...
submission.prototype.serializeSubmitDataList = function(submitDataList, serializationFormat, encoding, omitXMLDeclaration, standalone) {
	var serialization = "";
	...
	if (serializationFormat === "application/xml") {
		...
		if (!omitXMLDeclaration) {
			xmlDoc.insertBefore(
				xmlDoc.createProcessingInstruction("xml", "version=\"1.0\" encoding=\"" + encoding + "\"" + (standalone ? " standalone=\"" + standalone + "\"" : "")),
				xmlDoc.firstChild
			);
		}
