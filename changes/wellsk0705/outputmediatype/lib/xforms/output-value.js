...
XFormsOutputValue.prototype.getValue = function() {
    ...
    //    
    if (sTagNameLC === "label") {
        labels = NamespaceManager.getElementsByTagNameNS(this.element.parentNode, 
                                "http://www.w3.org/2002/xforms", "label");
        if (labels && labels.length > 0) {
            return labels[0].m_value.getValue();        
        } 
    }

    return sRet = document.all ?  this.element.innerText : this.element.textContent;  
};
