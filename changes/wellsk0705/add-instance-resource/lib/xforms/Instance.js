...
Instance.prototype.initialisedomWithURL = function ( domURL ) {
...
Instance.prototype.hasValidRootNode = function () {
    var coll = this.element.childNodes;
    for (var i=0; i < coll.length; i++) {
        if (coll[i].nodeType === DOM_ELEMENT_NODE) {
            return true;
        }
    }
    return false;   
...
Instance.prototype.initialisedom = function () {
    if (!this.m_oDOM) {       
        ...
        if (this.getAttribute("src")) { 
            this.initialisedomWithURL(this.getAttribute("src"));
        } else if (this.hasValidRootNode()) {
            this.parseInstance();           
        } else if (this.getAttribute("resource")) {
            this.initialisedomWithURL(this.getAttribute("resource"));
        } else {
           // throw xforms-link-exception;   
           UX.dispatchEvent(this.element, "xforms-binding-exception", 
                    false, true, true);        
        }
