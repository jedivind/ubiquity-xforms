...
FunctionCallExpr.prototype.xpathfunctions["id"] = function(ctx) {
    var evalCtx = this.args[0].evaluate(ctx);
    var ret = [];
    var ids;
    var nodeSet; 
    var nodeValue;
    ...
    if (evalCtx.type == 'node-set') {
        ...
        for (var i = 0; i < nodeSet.length; ++i) {
            ...
            for (var ii = 0; ii < nodeValue.length; ++ii) {
                ids.push(nodeValue[ii]);
            }
        }
    } else {
    ...
    for (var j = 0; j < ctxNodes.length; ++j) {
	    for (var k = 0; k < ids.length; ++k) {
	        oNode = ctxNodes[j].getElementById(ids[k]);
	        if (oNode) {
	            ret.push(oNode);
	        }
	    }
