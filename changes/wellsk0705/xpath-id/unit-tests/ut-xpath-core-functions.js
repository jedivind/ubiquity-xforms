...
var ctx = new ExprContext(
  xmlParse(
    "<test> \
      ...
      <numbers6 id='n6'> \
         <number xsi:type='xsd:ID'>a3</number> \
         <number xml:id='a2'>a2</number> \
         <number xsi:type='xsd:ID'>b6</number> \
         <number xsi:type='xsd:ID'>c12</number> \
      </numbers6> \
    </test>"
  )
);
...
suiteXPathCoreFunctions.add(
  new YAHOO.tool.TestCase({
    ...
    testIdSuccess : function () {
      var Assert = YAHOO.util.Assert; 
      var nodesetPropExpected;
      var nodesetPropReturned;
      
      // caution, the nodesets from the xml sample above contain carriage line returns as nodes
      // whereas, the id function will not return the carriage line returns (text nodes) with the nodesets
      nodesetPropExpected = evalXPath('/test/numbers6/number[2]').nodeSetValue();
      nodesetPropReturned = evalXPath("id('a2')").nodeSetValue();
      YAHOO.util.ObjectAssert.propertiesAreEqual(nodesetPropExpected, nodesetPropReturned);
      Assert.areEqual(nodesetPropExpected[0].getAttribute("xml:id"), nodesetPropReturned[0].getAttribute("xml:id"));
      
      nodesetPropExpected = evalXPath('/test/numbers6/number[1]').nodeSetValue();
      nodesetPropReturned = evalXPath("id('a3')").nodeSetValue();
      YAHOO.util.ObjectAssert.propertiesAreEqual(nodesetPropExpected, nodesetPropReturned);
      Assert.areEqual(nodesetPropExpected[0].getAttribute("xsi:type"), nodesetPropReturned[0].getAttribute("xsi:type"));
      
      nodesetPropExpected = evalXPath('/test/numbers6/number[2]').nodeSetValue();
      nodesetPropReturned = evalXPath("id('a2', /test/numbers6)").nodeSetValue();
      YAHOO.util.ObjectAssert.propertiesAreEqual(nodesetPropExpected, nodesetPropReturned);
      Assert.areEqual(nodesetPropExpected[0].getAttribute("xml:id"), nodesetPropReturned[0].getAttribute("xml:id"));
      
      nodesetPropExpected = evalXPath('/test/numbers6/number[3]').nodeSetValue();    
      nodesetPropReturned = evalXPath("id('b6', id('n6'))").nodeSetValue();
      YAHOO.util.ObjectAssert.propertiesAreEqual(nodesetPropExpected, nodesetPropReturned);
      Assert.areEqual(nodesetPropExpected[0].nodeValue, nodesetPropReturned[0].nodeValue);
    }
