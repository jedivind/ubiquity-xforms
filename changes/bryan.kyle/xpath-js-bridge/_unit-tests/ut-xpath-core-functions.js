	...
	new YAHOO.tool.TestCase(
		{
			...
			testMarshallingAndUnmarshallingOfStrings: function() {
				var Assert = YAHOO.util.Assert;
				Assert.areEqual('hello', evalXPath("My.echo('hello')").stringValue());
			},
			
			testMarshallingAndUnmarshallingOfNumbers: function() {
				var Assert = YAHOO.util.Assert;
				Assert.areEqual(1, evalXPath("My.echo(1)").numberValue());
			},
			
			testMarshallingAndUnmarshallingOfBooleans: function() {
				var Assert = YAHOO.util.Assert;
				Assert.areEqual(true, evalXPath("My.echo(true())").booleanValue());
			},
			
			testMarshallingAndUnmarshallingOfNodeSets: function() {
				var Assert = YAHOO.util.Assert;
				
				var nodeset = evalXPath("My.echo(/test/numbers/number)").nodeSetValue();
				Assert.areEqual(5, nodeset.length);
			},
			
			testFunctionAcceptingDataFromDom: function() {
				var Assert = YAHOO.util.Assert;
				Assert.areEqual(4, evalXPath("My.square(number(/test/numbers/number[2]/text()))").numberValue());
			},
			
			testFunctionChaining: function() {
				var Assert = YAHOO.util.Assert;
				Assert.areEqual('1', evalXPath("My.first(/test/numbers/number)/text()").stringValue());
			}
