	...
	if (this.m_sValue !== s) {
		if (this.element.parentNode.m_ancestorEventTarget) {
		  ...
		  //  future changes in value to be accurately reflected.
			this.element.parentNode.m_ancestorEventTarget.removeItem(this.element.parentNode,this.m_sValue);
			this.m_sValue = s;
			this.element.parentNode.m_ancestorEventTarget.addItem(this.element.parentNode,s);
...
Item.prototype.onContentReady = function () {
  ...
  this.findValueElement();
	...
	ownerSelect = this.getAncestorEventTarget();
	if (ownerSelect) {
		pThis = this;
