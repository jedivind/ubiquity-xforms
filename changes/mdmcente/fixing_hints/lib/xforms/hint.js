...
function Hint( el ) {
  ...
  if ( forAttr ) {
    ...
    var findForElement = el.ownerDocument.getElementById(forAttr);
    if ( findForElement ) {
       context = findForElement;
       context.insertBefore(el, context.firstChild);
    }
  }
...
Hint.prototype.handleEvent = function( evt ) {
  if ( evt.type === "xforms-hint" ) {
    if ( this.element.getAttribute("ref") ) {
    	this.element.innerHTML = this.element.getBoundNode(1).node.m_proxy.getValue();
    }
