...
Control.prototype.RetrieveValuePseudoElement = function()
{
	...
	return this.m_value;
                ...
                if (document.media != "print") {
                    ...
                    if (!this.m_value) { 
                        if (childNodes) {
                            for (i = 0; i < childNodes.length; ++i) {
                                if (DOM_TEXT_NODE === childNodes[i].nodeType) {
                                    ...
                                    if (i > 0) {
                                        childBeforeTextChild = childNodes[i - 1];
                                    }
                        ...
                        if (UX.isIE || UX.isWebKit) {  
                            ...
                            if (childBeforeTextChild) {
                                referenceNode = childBeforeTextChild.nextSibling;
                            }
                            ...
                            else if (childBeforeTextChild) {
                                referenceNode = childBeforeTextChild;
                                insertionPoint = "afterEnd";
                            }
                            else {
                            ...
                            if (childBeforeTextChild) {
                                this.m_value = childBeforeTextChild.nextSibling;
                            }
                        }
		...
		Control.prototype.rewire = function()
		{
			...
			/*
			 * [ISSUE] Would rather define this using
			 * mark-up.
			 */
			this.AddTVCListener();

			/*
			 * [ISSUE] This is essentially registering for the rewire and
			 * refresh events.
			 */
	...
	//					FormsProcessor.dispatchEvent(this.element,oEvt);
		...
		/*
		 * [ISSUE] This has evolved a little, and needs a good tidy up.
		 * The basic behaviour is to find a proxy node and then connect
		 * the control to it. Whether the proxy comes from a @ref or
		 * a @bind makes no difference.
		 */
		...
		Control.prototype.xrewire = function()
		{
			...
			/*
			 * Get the node this control is bound to (if any), but
			 * force the bindings to be reevaluated by clearing any
			 * proxy node.
			 *
			 * [TODO] This might need to be some kind of 'unwire', since
			 * we also need to null any cached nodelist.
			 */
			...
			/*
			 * If we have a @value then the 'bound node' will
			 * actually be a context node.
			 *
			 * [TODO] Call getEvaluationContext, instead of
			 * using the 'bound node' function, which should
			...
			/*
			 * If we have a node then we should bind our control
			 * to it.
			 */
			...
			/*
			 * [TEMP] This cannot be permanently removed
			 * until we can tell the difference between:
			 *
			 * (a) controls that *should* be bound but aren't,
			 * because there is no node;
			 *
			 * (b) controls that have no binding attributes,
			 * such as labels.
			 */
		...
		Control.prototype.refresh  = function()
		{
			...
			if (oProxy)
			{
				...
				/*
				 * Get the type of the node and pass the information
				 * to the control in case it needs to change its
