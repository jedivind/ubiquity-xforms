...
function getDispatchVariable(pThis, type)
{
	...
	for (var i = 0; i < pThis.element.childNodes.length; i++) {
		aChildNode = pThis.element.childNodes[i];
		if (NamespaceManager.compareFullName(aChildNode, type, "http://www.w3.org/2002/xforms")) {
		
			i = pThis.element.childNodes.length;
			if (UX.isIE) {
				sType = pThis.element.innerText;
			}
			else {
